...
if ($act == "add" || $act == "edit") {
	...
	if ($id) {
       ...
       $sql = "SELECT c.*, h.ban_nama, h.lpj_tgl, h.lpj_uraian, e.besaran_tapd FROM tbl_cair_bansos c left join tbl_bansos h on c.ban_kode=h.ban_kode left join tbl_eval_tapd_detail e on c.ban_kode=e.hib_kode left join tbl_eval_tapd f on e.kode=f.kode WHERE f.tipe='BANSOS' and c.id_cair='$id'";
...
<?
    ...
    if ($act=='do_update') {	    
        $sql = "UPDATE tbl_bansos SET lpj_tgl='".$f->preparedate($lpj_tgl)."', lpj_uraian='".trim(strip_tags($lpj_uraian))."' WHERE ban_kode=$id";
...
$sql="select c.*, h.ban_nama, h.lpj_tgl, h.lpj_uraian, e.besaran_tapd FROM tbl_cair_bansos c $cond1 $cond order by $order $sort";

...
$t->basicheader();
$f->checkaccess();
$t->title('Pelayanan &raquo; Hibah &raquo; Monitoring dan Evaluasi Hibah');
...
if ($act == "add" || $act == "edit") {
	...
	if ($id) {
        ...
        $sql = "SELECT a.*,b.hib_nama FROM tbl_monev_hibah a LEFT JOIN tbl_hibah b ON a.hib_kode=b.hib_kode WHERE a.hib_kode=$id";
    ...
    if ($id && $act == "add") unset($id, $tgl_monev);
    
    if (!$tgl_monev) $tgl_monev = date('Y-m-d');
    

?>
<script type="text/javascript" src="/monev_hibah.js"></script>
...
<input type="hidden" name="id" value="<?=$hib_kode?>">
  <tr>
    ...
    <td>
      <input type="text" name="tgl_monev" id="tgl_monev" value="<?=$f->convertdatetime(array("datetime"=>$tgl_monev))?>" class="easyui-validatebox" required="true" />
    </td>
  </tr>
  <tr>
    <td>Nama Penerima Hibah</td>
    <td>
      <input type="text" name="hib_nama" id="hib_nama" value="<?=$hib_nama?>" style="width:250px;" class="easyui-validatebox" required="true" />
      <input type="hidden" name="hib_kode" id="hib_kode" value="<?=$hib_kode?>" />
    </td>
  </tr>
  <tr>
  	<td>Hasil</td>
    <td><?=$f->selectListArray("hasil",array("Baik"=>"Baik", "Cukup Baik"=>"Cukup Baik", "Kurang"=>"Kurang"),$hasil)?></td>
  </tr>
  <tr>
  	<td>Uraian</td>
    <td>
...
<?
    ...
    if ($act=='do_update') {	    
        $sql = "UPDATE tbl_monev_hibah SET tgl_monev='".$f->preparedate($tgl_monev)."', hasil='$hasil', uraian='".trim(strip_tags($uraian))."' WHERE hib_kode=$id";
                
        $result=$db->Execute($sql);
        if(!$result){ print $db->ErrorMsg(); die(); }
    }
    else {
      $sql = "INSERT INTO tbl_monev_hibah (hib_kode, tgl_monev, hasil, uraian) VALUES
('$hib_kode','".$f->preparedate($tgl_monev)."','$hasil','".trim(strip_tags($uraian))."')";
	...
	/*
    $sql = "DELETE FROM tbl_monev_hibah_opd WHERE hib_kode=$id";
    $result=$db->Execute($sql);
    if(!$result){ print $db->ErrorMsg(); die(); }
	*/
...
$cond1 = " left join tbl_hibah h on m.hib_kode=h.hib_kode ";
...
$cond  .=" $rel (m.hib_kode = '$query' or h.hib_nama like '%$query%' or m.tgl_monev = '".$f->preparedate($query)."')";
}

$total = $f->count_total("tbl_monev_hibah m","$cond1 $cond"); 

$f->paging(array("link"=>$PHP_SELF."?query=$query&order=$order&sort=$sort&type=$type&act=","page"=>$page,"total"=>$total,"num"=>"$num","show_total"=>1));
$sql="select m.*, h.hib_nama from tbl_monev_hibah m $cond1 $cond order by $order $sort";
...
$_sort=($sort=='desc')?"asc":"desc"; 
	...
	while($val=$result->FetchRow()){
		...
		<tr bgcolor=$bgcolor>
			...
			<td valign=top>".$f->convertdatetime(array("datetime"=>$tgl_monev))."</td>

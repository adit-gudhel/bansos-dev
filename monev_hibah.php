...
if ($act == "add" || $act == "edit") {
	...
	if ($id) {
		...
		$sql = "SELECT c.*, h.hib_nama, h.mon_tgl, h.mon_hasil, h.mon_uraian, e.besaran_tapd FROM tbl_cair_hibah c left join tbl_hibah h on c.hib_kode=h.hib_kode left join tbl_eval_tapd_detail e on c.hib_kode=e.hib_kode left join tbl_eval_tapd f on e.kode=f.kode WHERE f.tipe='HIBAH' and c.id_cair='$id'";
    ...
    if ($id && $act == "add") unset($id, $mon_tgl);
...
<?
    ...
    if ($act=='do_update') {	   
		...
		$sql = "UPDATE tbl_hibah SET mon_tgl='".$f->preparedate($mon_tgl)."', mon_hasil='$mon_hasil', mon_uraian='".trim(strip_tags($mon_uraian))."' WHERE hib_kode=$id";
...
$sql="select c.*, h.hib_nama, h.mon_tgl, h.mon_hasil, h.mon_uraian, e.besaran_tapd FROM tbl_cair_hibah c $cond1 $cond order by $order $sort";

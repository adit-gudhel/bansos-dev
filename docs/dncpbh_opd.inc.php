...
$objPHPExcel->getProperties()->setCreator("Aditya Nursyahbani")
							 ...
							 ->setTitle("DNCPBH - OPD")
							 ->setSubject("DNCPBH OPD")
							 ->setDescription("Daftar Nominatif Calon Penerima Hibah - OPD")
							 ->setKeywords("dncpbh")
							 ->setCategory("report");
...
$sql = "SELECT nama,alamat,kelurahan,kecamatan,kota,propinsi,rencana_penggunaan,permohonan,hasil_evaluasi_opd,keterangan FROM v_dncpbh_opd WHERE kode='$kode' and jenis='Uang'";
...
$sql = "SELECT tgl_ba, opd FROM v_dncpbh_opd WHERE kode='$kode'";
$result=$db->Execute($sql);
$row = $result->FetchRow();
...
$sql = "SELECT nama,alamat,kelurahan,kecamatan,kota,propinsi,rencana_penggunaan,permohonan,hasil_evaluasi_opd,keterangan FROM v_dncpbh_opd WHERE kode='$kode' and (jenis='Barang' or jenis='Jasa')";
$result=$db->Execute($sql);

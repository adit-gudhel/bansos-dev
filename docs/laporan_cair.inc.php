...
$objPHPExcel->getProperties()->setCreator("Aditya Nursyahbani")
							 ...
							 ->setSubject("Laporan Pencairan")
							 ->setDescription("Laporan Pencairan Hibah dan Bantuan Sosial")
							 ->setKeywords("laporan, pencairan")
							 ->setCategory("report");
...
// Header
$AS->mergeCells('A1:E1');
$AS->setCellValue('A1', 'LAPORAN PENCAIRAN HIBAH');
$AS->mergeCells('A2:E2');
$AS->setCellValue('A2', '');

$arr_status = array("Belum Cair","Cair");
...
$sql = "SELECT v.nama, v.alamat, v.kelurahan, v.kecamatan, v.kota, v.propinsi, v.hasil_evaluasi_tapd as jumlah_uang,h.hib_cair as status_cair FROM v_dncpbh_tapd v LEFT JOIN tbl_hibah h on v.hib_kode=h.hib_kode WHERE YEAR(v.tgl_ba)='$thn' AND (v.status_opd = 1 AND v.status_tapd = 1) ORDER BY v.nama ASC";
...
while($row=$result->Fetchrow()){
	...
	$AS->setCellValue('E'.$start_row.'', $arr_status[$status_cair]);
...
$AS->getStyle('A4:E4')->applyFromArray($formatTableCenterB);
$AS->getStyle('A5:E5')->applyFromArray($formatTableCenterI);
...
// Header
$AS2->mergeCells('A1:E1');
$AS2->setCellValue('A1', 'LAPORAN PENCAIRAN BANTUAN SOSIAL');
$AS2->mergeCells('A2:E2');
$AS2->setCellValue('A2', '');
...
$sql = "SELECT v.nama, v.alamat, v.kelurahan, v.kecamatan, v.kota, v.propinsi, v.hasil_evaluasi_tapd as jumlah_uang,h.ban_cair as status_cair FROM v_dncpbs_tapd v LEFT JOIN tbl_bansos h on v.ban_kode=h.ban_kode WHERE YEAR(v.tgl_ba)='$thn' AND (v.status_opd = 1 AND v.status_tapd = 1) ORDER BY v.nama ASC";
$result=$db->Execute($sql);
...
while($row=$result->Fetchrow()){
	...
	$AS2->setCellValue('E'.$start_row.'', $arr_status[$status_cair]);
...
//Cair
$sql ="SELECT SUM(v.hasil_evaluasi_tapd) as total_cair FROM v_dncpbs_tapd v LEFT JOIN tbl_bansos h on v.ban_kode=h.ban_kode WHERE YEAR(v.tgl_ba)='$thn' AND (v.status_opd = 1 AND v.status_tapd = 1) AND h.ban_cair=1";
$result=$db->Execute($sql);
$row = $result->Fetchrow();

$AS2->setCellValue('B'.$summary_row.'', 'Total Cair');
$AS2->setCellValue('C'.$summary_row.'', $row['total_cair']);

$sql ="SELECT count(*) as jml_cair FROM v_dncpbs_tapd v LEFT JOIN tbl_bansos h on v.ban_kode=h.ban_kode WHERE YEAR(v.tgl_ba)='$thn' AND (v.status_opd = 1 AND v.status_tapd = 1) AND h.ban_cair=1";
$result=$db->Execute($sql);
$row = $result->Fetchrow();

$AS2->setCellValue('D'.$summary_row.'', 'Jumlah Cair');
$AS2->setCellValue('E'.$summary_row.'', $row['jml_cair']);


//Belum Cair
$sql ="SELECT SUM(v.hasil_evaluasi_tapd) as total_blm_cair FROM v_dncpbs_tapd v LEFT JOIN tbl_bansos h on v.ban_kode=h.ban_kode WHERE YEAR(v.tgl_ba)='$thn' AND (v.status_opd = 1 AND v.status_tapd = 1) AND h.ban_cair=0";
$result=$db->Execute($sql);
$row = $result->Fetchrow();

$AS2->setCellValue('B'.($summary_row + 1).'', 'Total Belum Cair');
$AS2->setCellValue('C'.($summary_row + 1).'', $row['total_blm_cair']);

$sql ="SELECT count(*) as jml_blm_cair FROM v_dncpbs_tapd v LEFT JOIN tbl_bansos h on v.ban_kode=h.ban_kode WHERE YEAR(v.tgl_ba)='$thn' AND (v.status_opd = 1 AND v.status_tapd = 1) AND h.ban_cair=0";
$result=$db->Execute($sql);
$row = $result->Fetchrow();

$AS2->setCellValue('D'.($summary_row + 1).'', 'Jumlah Belum Cair');
$AS2->setCellValue('E'.($summary_row + 1).'', $row['jml_blm_cair']);

$AS2->getStyle('A1:A2')->applyFromArray($formatJudul);
$AS2->getStyle('A4:E4')->applyFromArray($formatTableCenterB);
$AS2->getStyle('A5:E5')->applyFromArray($formatTableCenterI);

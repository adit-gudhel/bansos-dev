...
if ($act == "add" || $act == "edit") {
	...
	if ($id) {
        ...
        $sql = "SELECT a.*, b.hib_nama, c.besaran_tapd FROM tbl_pencairan a LEFT JOIN tbl_hibah b ON a.hib_kode=b.hib_kode LEFT JOIN tbl_eval_tapd_detail c ON a.hib_kode=c.hib_kode WHERE a.id_cair=$id";
...
<input type="hidden" name="id" value="<?=$id_cair?>">
  ...
  <tr>
    ...
    <td>
      ...
      <input type="hidden" name="jml_cair" id="jml_cair" value="<?=$jml_cair?>" />
    </td>
  </tr>
  <tr>
    <td colspan="2">&nbsp;</td>
...
<?
    ...
    if ($act=='do_update') {	    
        $sql = "UPDATE tbl_pencairan SET tgl_cair='".$f->preparedate($tgl_cair)."',jml_cair='$jml_cair',spph_no='$spph_no',spph_tgl='".$f->preparedate($spph_tgl)."',nphd_no='$nphd_no',
      ...
      $sql = "INSERT INTO tbl_pencairan (id_cair, tgl_cair, jml_cair, spph_no, spph_tgl, nphd_no, nphd_tgl, nphd_tentang, sp2d_no, sp2d_tgl, hib_kode, ctime, mtime, user) VALUES
('','".$f->preparedate($tgl_cair)."','$jml_cair','$spph_no','".$f->preparedate($spph_tgl)."','$nphd_no','".$f->preparedate($nphd_tgl)."','".trim($nphd_tentang)."','$sp2d_no','".$f->preparedate($sp2d_tgl)."','$hib_kode', NOW(), NOW(), '$login_full_name')";
	...
	$sql = "SELECT hib_kode FROM tbl_pencairan WHERE id_cair=$id";
    $result=$db->Execute($sql);
    if(!$result){ print $db->ErrorMsg(); die(); }
	...
	$sql = "DELETE FROM tbl_pencairan WHERE id_cair=$id";
    $result=$db->Execute($sql);
    if(!$result){ print $db->ErrorMsg(); die(); }
	
	/*
    $sql = "DELETE FROM tbl_pencairan_opd WHERE id_cair=$id";
    $result=$db->Execute($sql);
    if(!$result){ print $db->ErrorMsg(); die(); }
	*/
...
$cond1 = " left join tbl_hibah h on c.hib_kode=h.hib_kode left join tbl_eval_tapd_detail e on c.hib_kode=e.hib_kode ";
...
$total = $f->count_total("tbl_pencairan c","$cond1 $cond"); 

$f->paging(array("link"=>$PHP_SELF."?query=$query&order=$order&sort=$sort&type=$type&act=","page"=>$page,"total"=>$total,"num"=>"$num","show_total"=>1));

$sql="select c.*, h.hib_nama, e.besaran_tapd FROM tbl_pencairan c $cond1 $cond order by $order $sort";
...
$_sort=($sort=='desc')?"asc":"desc"; 
	...
	while($val=$result->FetchRow()){
		...
		<tr bgcolor=$bgcolor>
			...
			<td valign=top>".number_format($jml_cair,2,',','.')."</td>

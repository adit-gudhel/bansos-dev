...
<input type="hidden" name="old_id" value="<?=$no_rek?>">
  ...
  <tr>
  	<td>Keterangan</td>
    <td>
    	<textarea class="mceEditor" name="keterangan" id="keterangan" cols="45" rows="8" style="width:500px;" ><?=$uraian?></textarea>
    </td>
  </tr>
  <tr>
    <td colspan="2">&nbsp;</td>
...
<?
    ...
    if (!empty($id)) {
        ...
        $sql = "UPDATE tbl_rekening SET no_rek='$no_rek', uraian='$uraian', keterangan='".trim(strip_tags($keterangan))."' WHERE no_rek='$old_id'";
        ...
        $sql = "INSERT INTO tbl_rekening (no_rek, uraian, keterangan)
                VALUES ('$no_rek','$uraian','".trim(strip_tags($keterangan))."')";
        $result=$db->Execute($sql);
        if(!$result){ print $db->ErrorMsg(); die(); }
    }
	...
	<tr class=bgTitleTr>
		...
		<th class=white valign=top>Keterangan</th>

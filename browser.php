...
$sql = "select folder from tbl_access where user_id='$login_id'";
$result=$db->Execute($sql);
if (!$result) {print $db->ErrorMsg(); die();}
while($row=$result->FetchRow()){
    $dir_access[] = $row['folder'];
}
...
class EncodeExplorer
{
	...
	function readDir()
	{
		global $encodeExplorer, $dir_access;
		//
		...
		if($open_dir = @opendir($this->location->getFullPath()))
		{
			...
			while ($object = readdir($open_dir))
			{
                ...
                if($object != "." && $object != "..") 
				{
					...
					{
                        $folder = str_replace("./files/", "", $this->location->getDir(true, false, false, 0)."".$object);
                        #echo $folder."<p>";
                        
                        if (in_array($folder, $dir_access, true)) {
...
if($this->files)
{
	...
	foreach ($this->files as $file)
	{
		...
		print "<td class=\"name\">\n";
		print "\t\t<a target=_blank href=\"".$this->location->getDir(false, true, false, 0).$file->getNameEncoded()."\"";

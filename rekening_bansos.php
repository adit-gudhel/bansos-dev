...
$t->basicheader();
$f->checkaccess();
$t->title('Pelayanan &raquo; Bantuan Sosial &raquo; Input Data Rekening Bantuan Sosial');
...
if ($act == "add" || $act == "edit") {
	...
	if ($id) {
		...
		$sql = "SELECT a.ban_kode, a.nama, a.alamat, a.kelurahan, a.kecamatan, a.kota, a.propinsi, a.hasil_evaluasi_tapd as jumlah_uang, b.rek_anggaran as no_rek FROM v_dncpbs_tapd a LEFT JOIN tbl_bansos b ON a.ban_kode=b.ban_kode where (a.status_opd = 1 AND a.status_tapd = 1) AND a.ban_kode=$id";
...
?>
<script type="text/javascript" src="/rekening_bansos.js"></script>
...
<input type="hidden" name="id" value="<?=$ban_kode?>">
  ...
  <tr>
    <td>Alamat Penerima Bantuan Sosial</td>
    <td>
...
<?
    ...
    if ($act=='do_update') {	   
		...
		$sql = "UPDATE tbl_bansos SET rek_anggaran='$no_rek' WHERE ban_kode=$id";
...
$cond1 = " LEFT JOIN tbl_bansos b ON a.ban_kode=b.ban_kode ";
...
$total = $f->count_total("v_dncpbs_tapd a","$cond1 $cond"); 

$f->paging(array("link"=>$PHP_SELF."?query=$query&order=$order&sort=$sort&type=$type&act=","page"=>$page,"total"=>$total,"num"=>"$num","show_total"=>1));


$sql="SELECT a.ban_kode as id, a.nama, a.alamat, a.kelurahan, a.kecamatan, a.kota, a.propinsi, a.hasil_evaluasi_tapd as jumlah_uang, b.rek_anggaran FROM v_dncpbs_tapd a $cond1 $cond order by $order $sort";

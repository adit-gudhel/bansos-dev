...
$t->basicheader();
$f->checkaccess();
$t->title('Pelayanan &raquo; Bantuan Sosial &raquo; Daftar Penerima Bantuan Sosial');

$tahun = date('Y');

?>
<script type="text/javascript" src="/penerima_bansos.js"></script>
...
if ($generate && $thn!=='0'){
	$sql = "SELECT nama, alamat, kelurahan, kecamatan, kota, propinsi, hasil_evaluasi_tapd as jumlah_uang FROM v_dncpbs_tapd WHERE YEAR(tgl_ba)='$thn' AND (status_opd = 1 AND status_tapd = 1)";
	#echo $sql;
	$result=$db->Execute($sql);
	$row = $result->Fetchrow();
		
	if(empty($row['nama'])){
		echo "<p>Tidak ada penerima Bantuan Sosial pada tahun ".$thn."</p>";
	} else {
		
		//generate excel
		include 'docs/penerima_bansos.inc.php';
		
		require_once 'classes/PHPExcel/Classes/PHPExcel/IOFactory.php';
			
		$filename = "daftar-bansos-".$thn.".xls";

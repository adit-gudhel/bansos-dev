...
if ($act == "add" || $act == "edit") {
	...
	if ($id) {
        ...
        $sql = "SELECT a.*,b.ban_nama FROM tbl_monev_bansos a LEFT JOIN tbl_bansos b ON a.ban_kode=b.ban_kode WHERE a.ban_kode=$id";
...
<?
    ...
    if ($act=='do_update') {	    
        $sql = "UPDATE tbl_monev_bansos SET tgl_monev='".$f->preparedate($tgl_monev)."', hasil='$hasil', uraian='".trim(strip_tags($uraian))."' WHERE ban_kode=$id";
                
        $result=$db->Execute($sql);
        if(!$result){ print $db->ErrorMsg(); die(); }
    }
    else {
      $sql = "INSERT INTO tbl_monev_bansos (ban_kode, tgl_monev, hasil, uraian) VALUES
('$ban_kode','".$f->preparedate($tgl_monev)."','$hasil','".trim(strip_tags($uraian))."')";
	...
	/*
    $sql = "DELETE FROM tbl_monev_bansos_opd WHERE hib_kode=$id";
    $result=$db->Execute($sql);
    if(!$result){ print $db->ErrorMsg(); die(); }
	*/
...
$cond  .=" $rel (m.ban_kode = '$query' or h.ban_nama like '%$query%' or m.tgl_monev = '".$f->preparedate($query)."')";
}

$total = $f->count_total("tbl_monev_bansos m","$cond1 $cond"); 

$f->paging(array("link"=>$PHP_SELF."?query=$query&order=$order&sort=$sort&type=$type&act=","page"=>$page,"total"=>$total,"num"=>"$num","show_total"=>1));
$sql="select m.*, h.ban_nama from tbl_monev_bansos m $cond1 $cond order by $order $sort";

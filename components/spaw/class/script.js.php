...
SPAW_toggle_borders(editor,this[editor+'_rEdit'].document.body,null);
  ...
  */
  ...
  function SPAW_hyperlink_click(editor, sender)
  {
	...
	{
	  // edit hyperlink
      aProps.href = SPAW_stripAbsoluteUrl(a.href);
      aProps.name = a.name;
  ...
  function SPAW_image_prop_click(editor, sender)
  {
    ...
    {
      var iProps = {};
      iProps.src = SPAW_stripAbsoluteUrlFromImg(im.src);
      iProps.alt = im.alt;
  ...
  function SPAW_image_popup_click(editor, sender)
  {
    ...
    if(imgSrc != null)    
	{
		if (a)
		{
			// edit hyperlink
			a.href="#";
			a.onclick="window.open('<?php echo $spaw_img_popup_url?>?img_url="+SPAW_stripAbsoluteUrlFromImg(imgSrc)+"','Image','width=500,height=300,scrollbars=no,toolbar=no,location=no,status=no,resizable=yes,screenX=120,screenY=100');return false;";
		}
		else
		{
			var a;
  ...
  function SPAW_getHtmlValue(editor, thefield)
  {
    ...
    if(document.all['SPAW_'+editor+'_editor_mode'].value == 'design')
    {
	  ...
	  for (var i=0;i<aln;i++)
	  {
	  	links[i].href = SPAW_stripAbsoluteUrl(links[i].href);
		if (!links[i].href)
		{
	  ...
	  for (var i=0;i<aln;i++)
	  {
	  	imgs[i].src = SPAW_stripAbsoluteUrlFromImg(imgs[i].src);
	  }

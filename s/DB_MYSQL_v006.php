...
function Connect($server,$user,$pass,$dbname) {
   global $db;
   $this->conn = $db->Connect($server, $user, $pass, $dbname) or die($this->ShowBusy());
}
...
function SelectLimit($sql,$start=0,$numitems=0,$is_recordset=false) {
	...
	if ($numitems > 0) {
		$rs	= $db->SelectLimit($sql,$numitems,$start);
	} else {
		$rs	= $this->RunSql($sql);
	}
   $result  = array();
...
function GetFieldInfo($rs) {
	...
	$query_info = $rs->sql;
	$query_info_arr = split(" ",$query_info);
	//print "<pre>";  print_r($query_info_arr);
	for($i=0; $i<count($query_info_arr); $i++) {
		if ($query_info_arr[($i-1)] == "from") $table = $query_info_arr[$i];
	}
	...
	*/
	$fieldobject = $db->MetaColumns($table);
...
function OracleUpdateInsertSql($sql) {
	...
	while(list($fieldname,$clob) = each($this->tmpCLOB)) {
		...
		$db->InParameter($sth,$fieldvalue,$bindname,-1,OCI_B_CLOB);
	}
	$db->Execute($sth);
	$db->CommitTrans();
	unset($this->tmpCLOB);

...
if ($act == "add" || $act == "edit") {
	...
	if ($id) {
        ...
        $sql = "SELECT a.*,b.hib_nama FROM tbl_lpj_hibah a LEFT JOIN tbl_hibah b ON a.hib_kode=b.hib_kode WHERE a.hib_kode=$id";
...
<?
    ...
    if ($act=='do_update') {	    
        $sql = "UPDATE tbl_lpj_hibah SET tgl_lpj='".$f->preparedate($tgl_lpj)."', uraian='".trim(strip_tags($uraian))."' WHERE hib_kode=$id";
                
        $result=$db->Execute($sql);
        if(!$result){ print $db->ErrorMsg(); die(); }
    }
    else {
      $sql = "INSERT INTO tbl_lpj_hibah (hib_kode, tgl_lpj, uraian) VALUES
('$hib_kode','".$f->preparedate($tgl_lpj)."','".trim(strip_tags($uraian))."')";
	...
	/*
    $sql = "DELETE FROM tbl_lpj_hibah_opd WHERE hib_kode=$id";
    $result=$db->Execute($sql);
    if(!$result){ print $db->ErrorMsg(); die(); }
	*/
...
$cond  .=" $rel (m.hib_kode = '$query' or h.hib_nama like '%$query%' or m.tgl_lpj = '".$f->preparedate($query)."')";
}

$total = $f->count_total("tbl_lpj_hibah m","$cond1 $cond"); 

$f->paging(array("link"=>$PHP_SELF."?query=$query&order=$order&sort=$sort&type=$type&act=","page"=>$page,"total"=>$total,"num"=>"$num","show_total"=>1));
$sql="select m.*, h.hib_nama from tbl_lpj_hibah m $cond1 $cond order by $order $sort";

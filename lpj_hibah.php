...
$t->basicheader();
$f->checkaccess();
$t->title('Pelayanan &raquo; Hibah &raquo; Laporan Pertanggungjawaban Hibah');
...
if ($act == "edit") {
	...
	if ($id) {
        ...
        $sql = "SELECT c.*, h.hib_nama, h.lpj_tgl, h.lpj_uraian, e.besaran_tapd FROM tbl_cair_hibah c left join tbl_hibah h on c.hib_kode=h.hib_kode left join tbl_eval_tapd_detail e on c.hib_kode=e.hib_kode left join tbl_eval_tapd f on e.kode=f.kode WHERE f.tipe='HIBAH' and c.id_cair='$id'";
    ...
    if (!$lpj_tgl) $lpj_tgl = date('Y-m-d');
    

?>
<script type="text/javascript" src="/lpj_hibah.js"></script>
...
<input type="hidden" name="id" value="<?=$hib_kode?>" />
  ...
  <tr>
    <td>Tanggal LPJ</td>
    <td>
      <input type="text" name="lpj_tgl" id="lpj_tgl" value="<?=$f->convertdatetime(array("datetime"=>$lpj_tgl))?>" class="easyui-validatebox" required="true" />
    </td>
  </tr>
  <tr>
  	<td>Uraian</td>
    <td>
    	<textarea class="mceEditor" name="lpj_uraian" id="lpj_uraian" cols="45" rows="8" style="width:500px;" ><?=$lpj_uraian?></textarea>
    </td>
  </tr>
  <tr>
...
<?
    ...
    if ($act=='do_update') {	    
        $sql = "UPDATE tbl_hibah SET lpj_tgl='".$f->preparedate($lpj_tgl)."', lpj_uraian='".trim(strip_tags($lpj_uraian))."' WHERE hib_kode=$id";
...
$cond  .=" $rel (h.hib_nama like '%$query%' and f.tipe='HIBAH')";
}
...
$sql="select c.*, h.hib_nama, h.lpj_tgl, h.lpj_uraian, e.besaran_tapd FROM tbl_cair_hibah c $cond1 $cond order by $order $sort";
...
$_sort=($sort=='desc')?"asc":"desc"; 
	...
	<tr class=bgTitleTr>
		...
		<th class=white  valign=top>Tanggal LPJ</th>
	...
	while($val=$result->FetchRow()){
		...
		<tr bgcolor=$bgcolor>
			...
			<td valign=top>";
			if ($lpj_tgl){ echo $f->convertdatetime(array("datetime"=>$lpj_tgl));}
            echo "</td>
			<td valign=top>$lpj_uraian</td>
			";
